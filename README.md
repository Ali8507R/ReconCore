...
If no `-d` is supplied on the command line, the script walks the operator through setup instead of exiting. `interactive_setup()` prompts, in order, for:
...
`start_menu()` then presents a scan-mode menu once:
...
- `start_menu()` runs once rather than in a loop — an invalid selection exits the script instead of re-prompting.
